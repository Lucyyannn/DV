...
function _datevalues(d3, data) {
  return Array.from(d3.rollup(data, ([d]) => d.value, d => +d.date, d => d.name))
    .map(([year, data]) => [new Date(year, 0), data]) 
...
function _keyframes(d3, datevalues, k, rank) {
  ...
  for ([[ka, a], [kb, b]] of d3.pairs(datevalues)) {
    for (let i = 0; i < k; ++i) {
      const t = i / k;
      keyframes.push([
        new Date(ka.getFullYear() * (1 - t) + kb.getFullYear() * t, 0),
        rank(name => (a.get(name) || 0) * (1 - t) + (b.get(name) || 0) * t)
      ]);
    }
  }
  keyframes.push([new Date(kb.getFullYear(), 0), rank(name => b.get(name) || 0)]);
  return keyframes;
}
...
{
  const scale = d3.scaleOrdinal(["hsl(6, 66%, 51%)", "hsl(6, 66%, 61%)", "hsl(215, 100%, 65%)", "hsl(215, 100%, 55%)"]);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["category-brands.csv", {url: new URL("./files/race.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable().define(["md"], _1);
  main.variable().define("data", ["FileAttachment"], _data);
  main.variable().define("viewof replay", ["html"], _replay);
